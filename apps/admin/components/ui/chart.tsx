  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        ref={ref}
        className={cn(
          "flex aspect-video justify-center text-sm [&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-layer]:outline-none [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-[hsl(var(--chart-cursor))] [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-sector]:outline-none [&_.recharts-surface]:outline-none",
          className
        )}
        {...props}
      >
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex flex-wrap items-center justify-center gap-x-4 gap-y-2 w-full max-w-full min-w-0",
          verticalAlign === "top" ? "pb-3" : "pt-3",
          className
        )}
      >
        {payload.map((item) => {
          // For pie charts, item.value holds the series name (e.g. "Active")
          // which is the config key. For bar/line charts, use dataKey.
          const key = nameKey
            ? `${item.value}`
            : `${item.dataKey || "value"}`
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
