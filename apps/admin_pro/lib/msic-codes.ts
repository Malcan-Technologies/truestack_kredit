/**
 * MSIC 2008 sub-category codes (Malaysian Standard Industrial Classification).
 * Used to map SSM-returned business codes (e.g. "47413") to a human-readable
 * description so we can populate the borrower's free-text "nature of business"
 * field via the TrueSSM apply flow.
 *
 * AUTO-GENERATED from /Users/ivan/Documents/MSICSubCategoryCodes.json — do not
 * edit by hand; regenerate if the source list changes.
 *
 * NOTE: This is an intentional duplicate of
 * `apps/backend_pro/src/modules/truessm/msicCodes.ts`. The two apps don't share
 * a code package today, so we keep one canonical generated file per app — the
 * source `MSICSubCategoryCodes.json` is the single source of truth.
 */

export const MSIC_CODE_DESCRIPTIONS: Readonly<Record<string, string>> = Object.freeze({
...
/** Lookup the human-readable description for an MSIC code. */
export function lookupMsicDescription(code: string | null | undefined): string | null {
  if (!code) return null;
  const trimmed = code.trim();
  return MSIC_CODE_DESCRIPTIONS[trimmed] ?? null;
}
