# TrueKredit Pro — Borrower Product (Pinjocep)
...
**Status**: Phase 1 — Sign in & Sign up with Better Auth. backend_pro and borrower_pro (Pinjocep) setup.
...
Each Pro client (e.g. Pinjocep) has their own borrower frontend. Shared components (ShadCN, forms, flows) live at `apps/borrower_pro/components/` — no duplication per client.
...
│   │   │
│   │   └── Pinjocep/            # Client-specific app (this folder)
│   │       ├── app/
...
| `borrower_pro/Pinjocep/` | Pinjocep app — branding, pages, client-specific logic |
...
# Borrower_pro (Pinjocep)
cd apps/borrower_pro/Pinjocep
...
1. Copy `Pinjocep` folder (or the template used for new clients) → `ClientName/`
