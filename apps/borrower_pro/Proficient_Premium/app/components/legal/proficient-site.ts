/** Client-facing org details for Proficient Premium borrower portal. */
export const LENDER_NAME = "Proficient Premium";
export const LENDER_LEGAL_NAME = "Proficient Premium Sdn Bhd";
export const LENDER_EMAIL = "admin@proficientpremium.com";
export const LENDER_PHONE = "03 9770 2200";
/** Use with <a href={LENDER_PHONE_HREF}> for click-to-dial. */
export const LENDER_PHONE_HREF = "tel:+60397702200";
export const LENDER_WEB = "https://www.proficientpremium.com";
export const LENDER_SSM = "200301015650 (618070-W)";
export const LENDER_KPKT_LICENSE = "WL5127/10/01-11/030826";
export const LENDER_ADDRESS_LINES = [
  "B-21-1, Level 21, Tower B, Northpoint Mid Valley City",
  "No. 1, Medan Syed Putra Utara",
  "59200 Kuala Lumpur, Wilayah Persekutuan",
] as const;
export const LEGAL_LAST_UPDATED = "April 2026";
