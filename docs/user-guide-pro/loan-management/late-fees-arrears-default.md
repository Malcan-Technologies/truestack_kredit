...
A confirmation toast message shows how many loans were processed and how many fees were charged.
...
### Can I trigger late fee processing manually?

Yes. The **Process Late Fees** button on the Loans page runs the same calculation the cron job uses. It is safe to click multiple times — the system only charges fees for days that have not already been processed.
