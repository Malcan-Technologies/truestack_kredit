...
> **Pro-specific:** borrowers can also submit payment proofs through the borrower portal. Those submissions land in **Dashboard → TrueKredit Pro → Payment Approvals** and require a user with `payments.approve` to review before they are recorded against the loan. This page focuses on the admin-side recording flow. Permissions referenced below: `payments.record` to record, `payments.approve` to approve borrower-submitted payments.
...
- Borrower-submitted payments from the portal wait in the **Payment Approvals** queue until approved
