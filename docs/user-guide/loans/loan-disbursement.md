...
After a loan application is approved, the loan enters **Pending Disbursement** status. Before funds can be released, two documents must be prepared and uploaded:

1. **Loan Agreement** — Generated from the system and signed by both parties
2. **Stamp Certificate** — Obtained from LHDN (Inland Revenue Board)

Once both documents are uploaded, the admin can proceed with disbursement, which generates the repayment schedule and activates the loan.
