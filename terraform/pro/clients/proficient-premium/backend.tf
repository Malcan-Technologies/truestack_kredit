terraform {
  ...
  # Proficient Premium AWS account (873505479104) and region (ap-southeast-5).
  #
  # Recommended bootstrap (run once per AWS account, before `terraform init`):
  #   - S3 bucket: truestack-terraform-state-873505479104 (versioned + encrypted)
  #   - DynamoDB:  truestack-terraform-locks (LockID hash key, on-demand)
  backend "s3" {
    bucket                 = "truestack-terraform-state-873505479104"
